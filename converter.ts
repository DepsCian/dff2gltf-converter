...
export default async function convertDffToGlb(dff: Buffer, txd: Buffer): Promise<Document> {
  ...
  const { vertexInformation: rwVerticesArray, binMesh: rwBinMesh, textureMappingInformation: rwTextureInfo, normalInformation: rwNormalsArray } = rwGeometry;
  ...
  const verticesAccessor :Accessor = doc.createAccessor().setType("VEC3").setArray(vertices)
