...
export class DffConverter {
  ...
  async convertDffToGltf(): Promise<DffConversionResult> {
    ...
    const texturesMap: Map<String, Buffer> = new Map();
     ...
     /// TEXTURES
    try {
      const rwTxd: RwTxd = new TxdParser(this._txd).parse();
      ...
      console.error(e + 'Invalid .txd file.');
      return null;
    }
  
    // GEOMETRIES
    try {
      const rwDff = new DffParser(this._dff).parse();
    ...
    return new DffConversionResult(doc);
  }
}